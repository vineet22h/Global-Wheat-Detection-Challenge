...
def yolo_loss(args, num_classes, iou_loss_thresh, anchors):
    ...
    pred_sbbox = decode(conv_sbbox, anchors[0], 8, 1)
    pred_mbbox = decode(conv_mbbox, anchors[1], 16, 1)
    pred_lbbox = decode(conv_lbbox, anchors[2], 32, 1)
    sbbox_giou_loss, sbbox_conf_loss, sbbox_prob_loss = loss_layer(conv_sbbox, pred_sbbox, label_sbbox, true_sbboxes, 8, 1, iou_loss_thresh)
    mbbox_giou_loss, mbbox_conf_loss, mbbox_prob_loss = loss_layer(conv_mbbox, pred_mbbox, label_mbbox, true_mbboxes, 16, 1, iou_loss_thresh)
    lbbox_giou_loss, lbbox_conf_loss, lbbox_prob_loss = loss_layer(conv_lbbox, pred_lbbox, label_lbbox, true_lbboxes, 32, 1, iou_loss_thresh)
    giou_loss = sbbox_giou_loss + mbbox_giou_loss + lbbox_giou_loss
    ...
    # tf.print('total_loss: ', loss, 'giou_loss: ',giou_loss, 'conf_loss: ', conf_loss,'prob_loss: ', prob_loss)
